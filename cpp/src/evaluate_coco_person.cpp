    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--limit" && index + 1 < argc) {
            ...
            throw std::invalid_argument(
                "Usage: evaluate_coco_person [--limit N] [--warmup N]");
        }
    }
    return options;
}
...
void write_metrics_summary(
    ...
    std::ofstream file(path);
    if (!file) {
        throw std::runtime_error("Failed to open metrics summary file.");
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const auto image_paths =
            collect_images(image_directory, options.limit);
        const std::string suffix =
            options.limit == 0
                ? ""
                : "_" + std::to_string(image_paths.size()) + "_images";
        const std::filesystem::path prediction_path =
            "outputs/yolo11n/cpp_coco_val2017_person_predictions" + suffix +
            ".json";
        const std::filesystem::path metrics_path =
            "results/yolo11n/cpp_coco_person_metrics" + suffix + ".json";
        ...
        std::cout << "\nPredictions: " << predictions.size() << '\n'
                  ...
                  << "Metrics summary: " << metrics_path.string() << '\n';
